...
while (iargs < argc):
    ...
    if ((sys.argv[iargs] == '-h' or sys.argv[iargs] == '--help') and argc == 2):
        ...
        print (" [XGDtoolz - simple Tools]", 'green')
...
if ((data_dork != '' and data_page > 0 and data_file != '') or dump_module == 1):
        ...
        print        ("\n\n")
        print ("  __  ______ ____    42       _     ", 'blue')
        print ("  \ \/ / ___|  _ \  ___  _ __| | __ ", 'blue')
        print ("   \  / |  _| | | |/ _ \| '__| |/ / ", 'blue')
        print ("   /  \ |_| | |_| | (_) | |  |   <  ", 'blue')
        ...
        print ("  Contributor(s)/Source(s)", 'cyan')
        print        ("  - SQLmap ('agents file') - https://github.com/sqlmapproject/ ")
        print        ("  - ")
        print        ("\n")

        print (" [!] DISCLAIMER: A simple 'naive' tool to find SQLi Vulnerable websites in the wild via Google.", 'green')
        ...
        if (dump_module == 1):
            ...
            print (" [*] URL: "+data_url, 'cyan')
            print (" [*] Param: "+data_param, 'cyan')
            if (int(data_mod) < 3):
            ...
            if (data_bypass > 0):
                print (" [*] Warning: Bypass mode is active, it may not work...", 'red')
                
            print (" [*] let's try with [ "+data_dork+" ] Happy hunting ! ;) ", 'cyan')
